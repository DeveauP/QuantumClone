---
title: "Proof of concept: filtering pipeline"
author: "Paul Deveau"
date: "`r Sys.Date()`"
output: 
html_document:
...
Now let's select `r ndrivers` drivers among these:


````{r select_tp}
drivers<-sample(x = 1:nrow(noisy_data[[1]]),size = ndrivers,replace = FALSE)
...
filter_drivers<-function(cleaned.data,drivers){
  input<-list()
  
  for(i in 1:length(cleaned.data)){
    spare<-drivers[[i]][as.numeric(row.names(drivers[[i]]))>200,]
...
mb<-microbenchmark::microbenchmark(Paper_pipeline<-paper_pipeline(cleaned.data = filtered.data,
                                   ...
                                   Filter_drivers<-filter_drivers(cleaned.data = filtered.data,
                                                                  drivers = drivers_data),
                                   times = 5) # Change to 5 after debug
autoplot(mb)+theme_bw()

kable(Paper_pipeline$Drivers_clusts)
...
MaxDrivErr<-function(driv_cluster,cluster_cell,driver_data){
  ...
  xclone<-driver_data[[1]]$Cellularit/100
  yclone<-driver_data[[2]]$Cellularit/100
  xclust<-cluster_cell[[1]][as.numeric(as.character(driv_cluster))]
  yclust<-cluster_cell[[2]][as.numeric(as.character(driv_cluster))]
  return(max(sqrt((xclone - xclust)**2 + (yclone - yclust)**2)))
}
...
MaxDrivEr <-c(MaxDrivErr(driv_cluster = Paper_pipeline$Driver_clusts,
              ...
              MaxDrivErr(driv_cluster = All$cluster[All$filtered.data[[1]]$Start %in% drivers_data[[1]]$Start],
                         cluster_cell = All$EM.output$centers,
                         driver_data = drivers_data),
              MaxDrivErr(driv_cluster = Filter_drivers$cluster[Filter_drivers$filtered.data[[1]]$Start %in% drivers_data[[1]]$Start],
